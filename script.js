...
function updateRouteProgress() {
  const dot1 = document.getElementById("route-dot-1");
  const dot2 = document.getElementById("route-dot-2");
  if (!dot1 || !dot2) return;

  const now = new Date();
  const dayAtKinmen = new Date(tripInfo.startDate + "T00:00:00");   // 7/19 抵達金門
  const dayToXiamen = new Date(tripInfo.ferryToXiamen);              // 7/20 搭船至廈門
  const tripEnd = new Date(tripInfo.endDate + "T23:59:59");
  ...
  if (now < dayAtKinmen) {
    ...
    const total = tripEnd - dayToXiamen;
    const elapsed = now - dayToXiamen;
